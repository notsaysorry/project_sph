        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li
              ...
              :key="index"
            >
              {{ attr.split(":")[1] }}<i @click="removeProps(index)">×</i>
            </li>
          </ul>
        </div>

        <!--selector-->
        <SearchSelector @sendTmInfo="getTmInfo" @sendAttrInfo="getAttrInfo" />
...
export default {
  ...
  methods: {
    getData() {
      this.$store.dispatch("search/getGoodList", this.params);
    },
    removeCategoryName() {
    ...
    getTmInfo(tm) {
      console.log("index:");
      console.log(tm);
      this.params.trademark = `${tm.tmId}:${tm.tmName}`;
      this.getData();
    },
    removeTrademark() {
    ...
    changeSort(flag) {
      let orginFlag = this.params.order.split(":")[0];
      let orginSort = this.params.order.split(":")[1];
      let newOrder;
      if (orginFlag == flag) {
